...
-- users who booked flight on apr 2022

select u.segment, count(distinct u.User_id)  user_count
, count(distinct case when b.Line_of_business = 'Flight' 
		and b.Booking_date between '2022-04-01' and '2022-04-30'
        then b.User_id end) Flight_book_april
from 
user_table u left join booking_table b on b.User_id = u.User_id
group by u.segment;
...
-- days b/w first and last booking for each user

select user_id, min(booking_date) fast_book, max(booking_date) flast_book 
, datediff(max(booking_date), min(booking_date)) d
from booking_table
group by user_id;

-- count of flight and hotel for each user segment wise for year 2022
