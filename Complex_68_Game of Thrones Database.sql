...
-- for each region find house which has won maximum no of battles. 
...
with cte as(
select attacker_king king, region
from battle
...
, cte2 as(
select w.region, k.house, count(*) no_of_wins 
, rank() over(partition by w.region order by count(*) desc) as rn
from 
cte W inner join king k on  w.king = k.k_no
