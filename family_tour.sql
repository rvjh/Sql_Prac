...
-- max number of discounted tures for any family

select * from COUNTRIES where min_size < (select max(family_size) from FAMILIES);

select f.name, count(*) as cnt  from
FAMILIES f inner join COUNTRIES c on f.family_size between c.min_size and c.max_size
group by f.name
order by count(*) desc;